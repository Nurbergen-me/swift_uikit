...
//  Project20
//
//  Created by Nurbergen Yeleshov on 21.01.2021.
//
...
class GameScene: SKScene {
    ...
    func launchTimer(_ sender: UIAlertAction) {
        gameTimer = Timer.scheduledTimer(timeInterval: 6, target: self, selector: #selector(launchFireworks), userInfo: nil, repeats: true)
    }
    ...
    @objc func launchFireworks() {
        ...
        case 1:
            createFireworks(xMovement: 0, x: 512, y: bottomEdge)
            createFireworks(xMovement: 0 - 200, x: 512 - 200, y: bottomEdge)
            createFireworks(xMovement: 0 - 100, x: 512 - 100, y: bottomEdge)
            createFireworks(xMovement: 0 + 200, x: 512 + 200, y: bottomEdge)
            createFireworks(xMovement: 0 + 100, x: 512 + 100, y: bottomEdge)
    ...
    func explode(firework: SKNode) {
        ...
        run(SKAction.playSoundFileNamed("explode.m4a", waitForCompletion: false))
